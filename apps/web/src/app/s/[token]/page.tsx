...
function ShareHeader({ onPickKind, onCapture }: {
        ...
        <div className="p-6 space-y-4">
          ...
          {kind === "audio" && phase !== "error" && phase !== "starting" && (
            <div className="flex flex-col items-center gap-3 py-8">
              <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-colors ${phase === "recording" ? "bg-red-50 border-2 border-red-300" : "bg-muted"}`}>
                <Mic className={`w-10 h-10 transition-colors ${phase === "recording" ? "text-red-500" : "text-muted-foreground"}`} />
              </div>
              {phase === "recording" ? (
...
function ContribThumb({ contrib, onClick }: { contrib: Contribution; onClick: () => void }) {
  ...
  if (contrib.mediaType === "audio") {
    return (
      <button
        onClick={onClick}
        className="relative bg-white rounded-2xl text-left flex flex-col p-3 hover:opacity-90 transition-opacity"
        style={{
          aspectRatio: "192 / 139",
  ...
  if (contrib.mediaType === "video") {
    return (
      <button
        onClick={onClick}
        className="relative row-span-2 overflow-hidden rounded-2xl w-full hover:opacity-90 transition-opacity"
      >
...
function FullscreenViewer({ contrib, onClose }: { contrib: Contribution; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center" onClick={onClose}>
      ...
      <div className="max-w-2xl w-full px-4" onClick={(e) => e.stopPropagation()}>
        {contrib.mediaType === "photo" ? (
          ...
          <video src={contrib.mediaKey} controls autoPlay className="w-full rounded-lg" />
        ) : (
          <div className="bg-white rounded-lg p-6 space-y-3">
            <Music className="w-10 h-10 text-muted-foreground mx-auto" />
            <audio src={contrib.mediaKey} controls className="w-full" />
          </div>
        )}
...
export default function SharePage({ params }: { params: { token: string } }) {
  ...
  function handleUnlocked(viewerToken: string, unlockedCard: PublicCard) {
    ...
    // Load full data with contributions
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F5F5F0]">
            ...
            {contributions.length > 0 && (
              <div className="w-full">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2" style={{ gridAutoFlow: "row dense" }}>
                  {contributions.map((c) => (
                    <ContribThumb key={c.id} contrib={c} onClick={() => setActiveContrib(c)} />
                  ))}
                </div>
              </div>
            )}
      ...
      {activeContrib && <FullscreenViewer contrib={activeContrib} onClose={() => setActiveContrib(null)} />}
